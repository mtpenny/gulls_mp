...
lensdir=$base_dir`grep LENS_DIR $paramfile | awk -v FS='=' '{print $2}'`
lenslist=`grep LENS_LIST $paramfile | awk -v FS='=' '{print $2}'`
#echo  $base_dir
#echo $param_file_dir
#echo $paramfile
#echo $lensdir
srcdir=$base_dir`grep SOURCE_DIR $paramfile | awk -v FS='=' '{print $2}'`
srclist=`grep SOURCE_LIST $paramfile | awk -v FS='=' '{print $2}'`
sfdir=$base_dir`grep STARFIELD_DIR $paramfile | awk -v FS='=' '{print $2}'`
sflist=`grep STARFIELD_LIST $paramfile | awk -v FS='=' '{print $2}'`
planetdir=$base_dir`grep PLANET_DIR $paramfile | awk -v FS='=' '{print $2}'`
planetroot=`grep PLANET_ROOT $paramfile | awk -v FS='=' '{print $2}'`
obsdir=$base_dir`grep OBSERVATORY_DIR $paramfile | awk -v FS='=' '{print $2}'`
obslist=$obsdir`grep OBSERVATORY_LIST $paramfile | awk -v FS='=' '{print $2}'`
weatherdir=$base_dir`grep WEATHER_PROFILE_DIR $paramfile | awk -v FS='=' '{print $2}'`
...
band=`grep PRIMARYCOLOUR $paramfile | awk -v FS='=' '{print $2}'`
