...
double VBMicrolensing::BinaryMag(double a1, double q1, double y1v, double y2v, double RSv, double Tol, _sols_for_skiplist_curve** Images) {
	...
	stheta->astrox1 = 0.;
	stheta->astrox2 = 0.;
	y = y0 + VBcomplex(RSv * cos(thoff), RSv * sin(thoff));
	...
	do {
		...
		y = y0 + VBcomplex(RSv * cos(th), RSv * sin(th));
#ifdef _PRINT_TIMES
		tim0 = Environment::TickCount;
#endif
		//if (NPS == 44) {
...
#define _Jacobians2\
	dy = VBcomplex(-sin(theta->th), cos(theta->th))*coefs[23];\
...
_curve* VBMicrolensing::NewImages(VBcomplex yi, VBcomplex* coefs, _theta* theta) {
	static VBcomplex  y, yc, z, zc, J1, J1c, dy, dz, dJ, J2, J3, dza, za2, zb2, zaltc, Jalt, Jaltc, JJalt2;
	static VBcomplex zr[5] = { 0.,0.,0.,0.,0. };
...
// s: array of size n-1 with the separations in VBcomplex coordinates from the first mass.
////////// What does it make?
...
void VBMicrolensing::SetLensGeometry(int nn, double* pr) {
	...
	VBcomplex* s = (VBcomplex*)malloc(sizeof(VBcomplex) * (nn));

	for (int i = 0, j = 2; i < nn; ++i, j += 3) {
		q[i] = pr[j];
	}
	for (int i = 0, j = 0; i < nn; ++i, j += 3) {
		s[i] = VBcomplex(pr[j], pr[j + 1]);
	}
...
void VBMicrolensing::SetLensGeometry(int nn, double* q, VBcomplex* s) {
...
void VBMicrolensing::SetLensGeometry_spnp(int nn, double* q, VBcomplex* s) {
	static double sumq, qmin, Jac;
	static int iqmin, dg;
	static VBcomplex pbin[2], z, S2, fac;
	static int i, j;
...
void VBMicrolensing::SetLensGeometry_multipoly(int nn, double* q, VBcomplex* s) {
	static int j, i, x, k, p, dg;
	static double tempq, sumq;
	static VBcomplex temps, pbin[2];
...
double VBMicrolensing::MultiMag(double y1s, double y2s, double RSv, double Tol, _sols_for_skiplist_curve** Images) {
	...
	try {
		...
		stheta->astrox1 = 0.;
		stheta->astrox2 = 0.;
		y = y0 + VBcomplex(RSv * cos(thoff), RSv * sin(thoff)); // first image
...
#define _MJacobiansgood\
	dy = VBcomplex(-sin(theta->th)*rho, cos(theta->th)*rho);\
...
void VBMicrolensing::initroot() {
	static VBcomplex fac, fac2, z, S2;
	static double Jac;
	...
	_Jac
		while (Jac <= 0) {	// Check if main image has positive parity. Otherwise displace
			z = z + ((abs2(y) > 1.e-6) ? y : VBcomplex(0, 1));
			_Jac
		}
	init[n] = z;
...
int VBMicrolensing::froot(VBcomplex zi) {
	static VBcomplex z, zc, S1, S2, S2v, S3, zo, zo2, epso, epsbase, epsn, epsl, gradL, zl, fac, fac2, dz, dzo, TJold, TJnew, Lv, den;
...
_curve* VBMicrolensing::NewImages(_theta* theta) {
	...
	static VBcomplex z, zc, dy, dz, J2, J3, Jalt, JJalt2, Jaltc, J1c2;
...
void VBMicrolensing::initrootpoly() {
	static double mrt;
	static VBcomplex dev, dev2, zplus, shear, alpha0;
	static int ir;
	zplus = y;
	ir = nroots - 1;
	for (int i = 0; i < n; i++) {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < 2 * (n - 1); j++) {
			dev = M_PI * (j / (n - 1.) + 0.5);
			zr[ir] = a[i] + dev2 * VBcomplex(cos(dev.re), sin(dev.re));
			ir--;
		}
	}
}
...
int VBMicrolensing::findimagepoly(int i) {
	static VBcomplex z, zc, yc, LL, zo, delta, lambda;
...
_curve* VBMicrolensing::NewImagespoly(_theta* theta) {
	static VBcomplex  yc, z, zc, zo, delta, dy, dz, J2, J3, Jalt, Jaltc, JJalt2, LL, J1c2, dzita;
...
void VBMicrolensing::TripleAstroLightCurve(double* pr, double* ts, double* mags, double* c1s, double* c2s, double* c1l, double* c2l, double* y1s, double* y2s, int np) {
	...
	s[1] = s[0] * q[0];
	s[0] = -q[1] * s[0];
	s[2] = exp(pr[7]) * VBcomplex(cbeta, sbeta) + s[0];
...
void VBMicrolensing::LightCurve(double* pr, double* ts, double* mags, double* y1s, double* y2s, int np, int nl) {
	...
	VBcomplex* s = (VBcomplex*)malloc(sizeof(VBcomplex) * (nl));
	...
	s[0] = VBcomplex(0, pr[3]);
	for (int i = 1, j = 1; i < nl; ++i, j += 3) {
		s[i] = VBcomplex(pr[j + 3], pr[j + 4]);
	}
...
void VBMicrolensing::polyproduct(VBcomplex* __restrict p1, int n1, VBcomplex* __restrict p2, int n2, VBcomplex* __restrict pdest) {
...
void VBMicrolensing::copypol(VBcomplex* __restrict p1, int n1, VBcomplex* __restrict pdest) {
	for (int i = 0; i <= n1; i++) {
...
void VBMicrolensing::change_n(int nn) {
	...
	nnm1 = n2 - n;
	nroots = 2 * n2 + 1;
	coefs = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots + 1));
	pmza = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
	pmza2 = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
	pyaza = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
	ppmy = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
	za = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
	za2 = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
	for (int i = 0; i < n; i++) {
		pmza[i] = (VBcomplex*)malloc(sizeof(VBcomplex) * n);
		pmza2[i] = (VBcomplex*)malloc(sizeof(VBcomplex) * (2 * n - 1));
		pyaza[i] = (VBcomplex*)malloc(sizeof(VBcomplex) * (n + 1));
		ppmy[i] = (VBcomplex*)malloc(sizeof(VBcomplex) * (nnm1 + 1));
		za[i] = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots));
		za2[i] = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots));
	}

	pza = (VBcomplex*)malloc(sizeof(VBcomplex) * (n + 1));
	pza2 = (VBcomplex*)malloc(sizeof(VBcomplex) * (2 * n + 1));
	pdum = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots + 1));
	ppy = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots + 1));

	zr = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots));
	zcr = (VBcomplex*)malloc(sizeof(VBcomplex) * (2 * n));
	for (int i = 0; i < nroots; i++) {
		zr[i] = 0;
	}
	for (int i = 0; i < 2 * n; i++) {
	...
	J1 = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots));
	J1c = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots));
	zaltc = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots));
	worst = (int*)malloc(sizeof(int) * (nroots));
	pert = (VBcomplex*)malloc(sizeof(VBcomplex) * n);

	m = (double*)malloc(sizeof(double) * n);
	a = (VBcomplex*)malloc(sizeof(VBcomplex) * n);
	prodevs = (double*)malloc(sizeof(double) * n);
	devs = (VBcomplex*)malloc(sizeof(VBcomplex) * n);
	init = (VBcomplex*)malloc(sizeof(VBcomplex) * (n + 1));
	centralimages = (VBcomplex*)malloc(sizeof(VBcomplex) * (nnm1) / 2);
	errs = (double*)malloc(sizeof(double) * nroots);
	newseeds = (VBcomplex*)malloc(sizeof(VBcomplex) * (2 * nroots));
	grads = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots));
	S2s = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots));
	S3s = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots));
	S4s = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots));
...
void VBMicrolensing::change_n_mp(int nn) {
	...
	s_sort = (VBcomplex*)malloc(sizeof(VBcomplex) * n);
	y_mp = (VBcomplex*)malloc(sizeof(VBcomplex) * n);

	m_mp = (double**)malloc(sizeof(double*) * n);
	a_mp = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
	for (int i = 0; i < n; i++) {
		m_mp[i] = (double*)malloc(sizeof(double) * n);
		a_mp[i] = (VBcomplex*)malloc(sizeof(VBcomplex) * n);
	}

	coefs_mp = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
	pza_mp = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
	ppy_mp = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
	for (int j = 0; j < n; j++) {
		coefs_mp[j] = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots + 1));
		pza_mp[j] = (VBcomplex*)malloc(sizeof(VBcomplex) * (n + 1));
		ppy_mp[j] = (VBcomplex*)malloc(sizeof(VBcomplex) * (nroots + 1));
	}

	pmza_mp = (VBcomplex***)malloc(sizeof(VBcomplex**) * n);
	pyaza_mp = (VBcomplex***)malloc(sizeof(VBcomplex**) * n);
	ppmy_mp = (VBcomplex***)malloc(sizeof(VBcomplex**) * n);
	for (int j = 0; j < n; j++) {
		pmza_mp[j] = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
		pyaza_mp[j] = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
		ppmy_mp[j] = (VBcomplex**)malloc(sizeof(VBcomplex*) * n);
		for (int i = 0; i < n; i++) {
			pmza_mp[j][i] = (VBcomplex*)malloc(sizeof(VBcomplex) * n);
			pyaza_mp[j][i] = (VBcomplex*)malloc(sizeof(VBcomplex) * (n + 1));
...
void VBMicrolensing::polycritcoefficients(VBcomplex eiphi) {
	static int dg;
	static VBcomplex pbin[3];
...
void VBMicrolensing::solve_quadratic_eq(VBcomplex& x0, VBcomplex& x1, VBcomplex* poly) {
	static VBcomplex a, b, c, b2, delta;
	a = poly[2];
	b = poly[1];
	c = poly[0];
	b2 = b * b;
...
void VBMicrolensing::solve_cubic_eq(VBcomplex& x0, VBcomplex& x1, VBcomplex& x2, VBcomplex* poly) {
	...
	static VBcomplex zeta = VBcomplex(-0.5, 0.8660254037844386);
	static VBcomplex zeta2 = VBcomplex(-0.5, -0.8660254037844386);
	static double third = 0.3333333333333333;
	static VBcomplex s0, s1, s2;
	static VBcomplex E1; //x0+x1+x2
	static VBcomplex E2; //x0*x1+x1*x2+x2*x0
	static VBcomplex E3; //x0*x1*x2
	static VBcomplex A, B, a_1, E12, delta, A2;

	static VBcomplex val, x;
	a_1 = 1 / poly[3];
	E1 = -poly[2] * a_1;
	E2 = poly[1] * a_1;
	E3 = -poly[0] * a_1;
...
void VBMicrolensing::cmplx_laguerre(VBcomplex* poly, int degree, VBcomplex* root, int& iter, bool& success) {
	//Subroutine finds one root of a VBcomplex polynomial using
	...
	static VBcomplex p, dp, d2p_half; //value of polynomial, 1st derivative, and 2nd derivative
	static int i, j, k;
	static bool good_to_go;
	static VBcomplex denom, denom_sqrt, dx, newroot;
	static double ek, absroot, abs2p;
	static VBcomplex fac_newton, fac_extra, F_half, c_one_nth;
	static double one_nth, n_1_nth, two_n_div_n_1;
	static VBcomplex c_one = VBcomplex(1, 0);
	static VBcomplex zero = VBcomplex(0, 0);
	...
	for (i = 1; i <= MAXIT; i++) {
		...
		if (denom == 0) {
			dx = (absroot + 1.0) * expcmplx(VBcomplex(0.0, FRAC_JUMPS[i % FRAC_JUMP_LEN] * 2 * M_PI));
		}
		else {
			dx = fac_newton / denom;
		}
...
void VBMicrolensing::cmplx_newton_spec(VBcomplex* poly, int degree, VBcomplex* root, int& iter, bool& success) {
	//Subroutine finds one root of a VBcomplex polynomial
	...
	static VBcomplex p; //value of polynomial
	static VBcomplex dp; //value of 1st derivative
	static int i, k;
	static bool good_to_go;
	static VBcomplex dx, newroot;
	static double ek, absroot, abs2p;
	static VBcomplex zero = VBcomplex(0, 0);
	static double stopping_crit2;
	...
	for (i = 1; i <= MAXIT; i++) {
		...
		if (dp == zero) {
			//problem with zero
			dx = (abs(*root) + 1.0) * expcmplx(VBcomplex(0.0, FRAC_JUMPS[i % FRAC_JUMP_LEN] * 2 * M_PI));
		}
		else {
...
void VBMicrolensing::cmplx_laguerre2newton(VBcomplex* poly, int degree, VBcomplex* root, int& iter, bool& success, int starting_mode) {
	//Subroutine finds one root of a VBcomplex polynomial using
	...
	static VBcomplex p; //value of polynomial
	static VBcomplex dp; //value of 1st derivative
	static VBcomplex d2p_half; //value of 2nd derivative
	static int i, j, k;
	static bool good_to_go;
	//VBcomplex G, H, G2;
	static VBcomplex denom, denom_sqrt, dx, newroot;
	static double ek, absroot, abs2p, abs2_F_half;
	static VBcomplex fac_netwon, fac_extra, F_half, c_one_nth;
	static double one_nth, n_1_nth, two_n_div_n_1;
	static int mode;
	static VBcomplex c_one = VBcomplex(1, 0);
	static VBcomplex zero = VBcomplex(0, 0);
	...
	for (;;) { //infinite loop, just to be able to come back from newton, if more than 10 iteration there
		...
		if (mode >= 2) {//Laguerre's method
			...
			c_one_nth = VBcomplex(one_nth, 0.0);
				...
				if (denom == zero) {//test if demoninators are > 0.0 not to divide by zero
					dx = (abs(*root) + 1.0) + expcmplx(VBcomplex(0.0, FRAC_JUMPS[i % FRAC_JUMP_LEN] * 2 * M_PI)); //make some random jump
				}
				else {
					dx = fac_netwon / denom;
				}
		...
		if (mode == 0) { // Newton's Method
				...
				if (dp == zero) {
					dx = (abs(*root) + 1.0) * expcmplx(VBcomplex(0.0, 2 * M_PI * FRAC_JUMPS[i % FRAC_JUMP_LEN])); // make a random jump
				}
				else {
					dx = p / dp;
				}
...
double VBMicrolensing::BinaryMag0_shear(double a1, double q1, double y1v, double y2v, double K1, double G1, double Gi, _sols **Images) {
	...
	static VBcomplex a, q, m1, m2, y, yc, mdiff, mtot, K, G;
	static double av = -1.0, qv = -1.0, Kv = -1.0, Gv = -1.0, Giv = -1.0, cq;
	static VBcomplex  coefs[28], d1, d2, dy, dJ, dz;
	double Mag = -1.0;
	_theta *stheta;
	_curve *Prov, *Prov2;
	_point *scan1, *scan2;
...
_curve *VBMicrolensing::NewImages_shear(VBcomplex yi, VBcomplex  *coefs, _theta *theta) {
    // by Lawrence Pierson (https://github.com/alpv95)
	static VBcomplex  y, yc, z, zc, Gc, J1, J1c, dy, dz, dJ,J2,J3,dza,za2,zb2,zaltc,Jalt,Jaltc,JJalt2;
	static VBcomplex zr[9] = { 0.,0.,0.,0.,0.,0.,0.,0.,0.};
